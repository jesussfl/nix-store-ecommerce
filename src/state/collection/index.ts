...
import { GraphQLTypes, SortOrder } from '@/zeus'
...
>((initialState) => {
  ...
  const initialSort = initialState.sort
    ? initialState.sort
    : { key: 'title', direction: SortOrder.ASC }
  ...
  const [q, setQ] = useState<string | undefined>(
    initialState.searchQuery ? initialState.searchQuery : undefined
  )
  ...
  const applyFilter = async (
    ...
    const newState = {
      ...filters,
      [group.id]: [...(filters[group.id] || []), value.id],
    }
    ...
    if (url.searchParams.get(group.name)) {
      url.searchParams.set(
        group.name,
        `${url.searchParams.get(group.name)},${value.name}`
      )
  ...
  const removeFilter = async (
    ...
    const newState = {
      ...filters,
      [group.id]: filters[group.id].filter((f) => f !== value.id),
    }
