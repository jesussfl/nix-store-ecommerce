...
export const DashboardSidebar: FC = function () {
  ...
  const filterMenuItems = (items: SideMenuItem[]) => {
    return items.filter((item) => {
      if (item.requiredPermissions) {
        ...
        return item.requiredPermissions.some((section) =>
  ...
  return (
    <Sidebar
      ...
      theme={customTheme}
      className={twMerge(
        'fixed inset-y-0 left-0 z-20 mt-0 flex h-full shrink-0 flex-col bg-dark duration-75 lg:flex',
    ...
    >
          ...
          {menuItems.map((item, idx) => {
            if (item.submenu) {
              return (
                ...
                >
                  <div className={`flex gap-4 rounded-md bg-[#262627]`}>
                    <div
      ...
      >
        <div
          ...
          } rounded-md border border-[#49494b] bg-[#262627] py-2`}
        >
          <UserNav />
